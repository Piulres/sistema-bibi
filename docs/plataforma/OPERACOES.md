...
Exemplo atual em produção: **`v3.0.1`** — **Sistema Bibi - ServiceOS** (qualidade multi-nicho + regras da agenda; empilha PWA `/instalar` + CEDIG em modo operação). Ver [`RELEASES.md`](../versoes/RELEASES.md).
...
**Produção:** https://sistema-bibi.netlify.app · **v3.0.1** · modo **operação** (CEDIG) · Stop builds **ON**  
