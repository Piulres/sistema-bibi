# Sistema Bibi - ServiceOS v3.0.1 — Base de Conhecimento (NotebookLM)

Documento consolidado para ingestão em ferramentas de RAG (NotebookLM, etc.).
Última atualização: reflete **ServiceOS v3.0.1** (qualidade multi-nicho + regras da agenda; PWA + multi-nicho desde v2.0), **white label** (tema escuro, logos Blobs),
...
- Deploy Netlify — **pacotes fechados** (não deploy a cada merge). Produção **v3.0.1**:
...
*Fim do documento — Sistema Bibi - ServiceOS v3.0.1 (corpus RAG; base multi-nicho desde v2.0)*
