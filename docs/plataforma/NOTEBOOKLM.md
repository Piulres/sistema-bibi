# Sistema Bibi - ServiceOS v3.0.16 — Base de Conhecimento (NotebookLM)

Documento consolidado para ingestão em ferramentas de RAG (NotebookLM, etc.).
Última atualização: reflete **ServiceOS v3.0.16** (PWA + multi-nicho desde v2.0), **white label** (tema escuro, logos Blobs),
reagendar beneficiário, Assistente Fase 0, auditoria RBAC fase 2, estoque Fase 2 UI, jornada narrativa do consultório,
...
Infraestrutura **SaaS multi-tenant multi-segmento** (ServiceOS; marco multi-nicho v2.0). Cada operação é um **tenant**
...
- Deploy Netlify — **pacotes fechados** (não deploy a cada merge). Produção **v3.0.16**:
  https://sistema-bibi.netlify.app (`docs/plataforma/DEPLOY_NETLIFY.md`, `docs/versoes/RELEASES.md`, `docs/plataforma/OPERACOES.md`).
  Pacote v3.0.16: reagendar beneficiário, Assistente Fase 0, auditoria RBAC fase 2, estoque Fase 2 UI.
...
*Fim do documento — Sistema Bibi - ServiceOS v3.0.16 (corpus RAG; base multi-nicho desde v2.0)*
