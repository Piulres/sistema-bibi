...
## 7. UI migrada para `useLabels()` (v3.0.1)
...
| Views dos portais (faturamento, agenda, PJ, prestador, beneficiário) | ✅ v3.0.1 — ver [`V3_0.md`](V3_0.md) |
| Breadcrumbs de histórico do paciente | ✅ `buildPatientHistoryBreadcrumbs` |
| `routes.ts` (nav estática interna) | 🟡 strings fixas |
...
| [`RELEASES.md`](RELEASES.md) | **Fonte de verdade de produção** (produção: v3.0.0 · `main`: v3.0.1) |
...
1. ~~Migrar strings fixas restantes para `useLabels()`~~ — views/breadcrumbs concluídos em v3.0.1; resíduo em `routes.ts` e APIs.
