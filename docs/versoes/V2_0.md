...
| **Produção atual** | **v2.0.0** — deploy `6a3a9973` @ `49edb90` — ver [`RELEASES.md`](RELEASES.md) |
| **Data desta revisão** | 23/06/2026 |
...
| Resolução server | `src/lib/segment/resolve.ts` | `?tenant=` → cookie → host → `?niche=` |
...
| Arquivo | Cobertura |
|---------|-----------|
| `tests/unit/segment.test.ts` | Resolução por slug (`?tenant=`), prioridade de query params |
| `tests/api/audit-pricing.test.ts` | CRUD pricing rules + timeline |
| `tests/integration/pricing-db.test.ts` | `computePrice` com escopo `tenantId` |
...
5. ~~Fechar pacote v2.0~~ — concluído em 23/06/2026; próximos pacotes seguem fluxo em [`RELEASES.md`](RELEASES.md).
