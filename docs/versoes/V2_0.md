...
| **Produção atual** | Ver [`RELEASES.md`](RELEASES.md) |
...
Cada **nicho** (`Tenant.niche`) define o glossário padrão; cada **tenant** (`Tenant.slug`) é uma clínica/cliente com branding e catálogo próprios. Vários tenants podem compartilhar o mesmo nicho.

| `niche` | Setor | Tenant demo (seed) | Login interno |
|---------|-------|-------------------|---------------|
| `MEDICAL` | Saúde | Clínica Horizonte (`horizonte`) | `faturamento@bibi.health` |
| `MEDICAL` | Saúde | CEDIG Cruzeiro (`cedig`) — piloto operação | `operacao@cedig.demo` |
| `MEDICAL` | Saúde | VitaCare (`vitacare`) — white-label | `operacao@vitacare.demo` |
| `VET` | Veterinária | PetCare (`petcare`) | `operacao@petcare.demo` |
| `DENTAL` | Odontologia | Smile Odonto (`smile`) | `operacao@smile.demo` |
| `LEGAL` | Jurídico | Lex & Partners (`lex`) | `operacao@lex.demo` |
| `SPA` | Bem-estar | Zen Studio (`zen`) | `operacao@zen.demo` |
| `EDUCATION` | Educação | EduPrime (`eduprime`) | `operacao@eduprime.demo` |
| `CONSTRUCTION` | Engenharia | Build Corp (`build`) | `operacao@build.demo` |

Senha demo: **`bibi123`**.

Preview local da landing: `/?tenant=cedig`, `/?niche=VET`, `/?niche=LEGAL`, etc. CEDIG **não** é nicho — é tenant `MEDICAL`; ver [`clientes/cedig/README.md`](../clientes/cedig/README.md).
...
| [`RELEASES.md`](RELEASES.md) | **Fonte de verdade de produção** (hoje v2.3.0) |
