...
- Seed `/?tenant=cedig`:
  - **Demo** — equipe, catálogo, 4 portais, histórico de homologação (`portalMass` + `seedHistory`)
  - **Operação** — equipe + catálogo apenas (greenfield; sem PJ/pacientes/histórico)
- CEDIG no bootstrap de operação + endpoint `provision-cedig` para bases Blobs legadas
