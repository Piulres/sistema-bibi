...
> Pacote **v2.6.0** — CEDIG fase 2 (pontes gestão → PPU) **pendente deploy**.  
> Produção ainda em **v2.4.0**. (v2.5 login tenant/portal pode integrar em paralelo.)
...
| **Release código (`dev` / PR)** | **v2.6.0** — pontes CEDIG · aguarda merge + deploy |
...
|----------|--------|----------|
| **Integração** | `dev` | **v2.6.0** (pendente merge/PR) |
| **Release** | `main` | **v2.4.0** em prod · **v2.6.0** após merge + deploy |
...
| `#novidades` | current **2.6.0** (após merge; prod ainda 2.4.0 até deploy) |
...
|-----|---------------|----------|
| **`v2.6.0`** | _(após deploy)_ | CEDIG fase 2 — ponte PPU + export |
...
### `v2.6.0` — Sistema Bibi - ServiceOS (CEDIG fase 2 · pontes) *(pendente deploy)*

| Campo | Valor |
|-------|-------|
| **Tag git** | `v2.6.0` |
| **Commit publicado** | _(preencher pós-deploy)_ |
| **Doc** | [`V2_6.md`](V2_6.md) · [`clientes/cedig/FASE_2.md`](../clientes/cedig/FASE_2.md) |
| **Inclui** | Ponte lançamento→agenda/PPU/fatura · Lançar na gestão · export Excel · READONLY write guard |
| **Publicado em** | _(aguardando `netlify deploy --prod`)_ |

---

### `v2.4.0` — Sistema Bibi - ServiceOS (CEDIG gestão clínica) *(substituído por v2.6.0 após deploy)*
