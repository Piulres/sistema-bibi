...
Última revisão: **ServiceOS v2.2** — tour guiado de onboarding nos 4 portais, labels por tenant, landing segmentada.
...
0.1. [Tour guiado de onboarding (v2.2)](#01-tour-guiado-de-onboarding-v22)
...
| Wizard de contratação (escolher nicho) | ❌ roadmap |
| Tour guiado na primeira visita (product tour) | ✅ v2.2 — ver [§0.1](#01-tour-guiado-de-onboarding-v22) |

---

## 0.1 Tour guiado de onboarding (v2.2)

Product tour automático nos **quatro portais autenticados** — distinto do wizard de contratação (roadmap acima).

| Aspecto | Experiência do usuário |
|---------|------------------------|
| **Quando** | Primeira visita a cada portal (interno, prestador, PJ, beneficiário) |
| **Visual** | Spotlight no elemento, hotspot pulsante, tooltip com título e descrição |
| **Conteúdo** | Textos com vocabulário do nicho (`labels.patient`, `labels.appointment`…) |
| **Contexto** | Passos extras conforme a rota (ex.: faturamento em `/interno`, agendar em `/beneficiario/agendar`) |
| **Reinício** | Botão **Tour** no header do portal |
| **Pular** | Botão **Pular**, **×** ou tecla Esc — tour pode reaparecer na próxima visita |
| **Conclusão** | Último passo ou **Concluir** — não repete automaticamente naquele portal |

**Validar localmente:** login em qualquer portal demo → aguardar ~1 s ou clicar **Tour**. Limpar progresso: DevTools → Application → `localStorage` → remover chave `bibi_onboarding`.

Detalhes técnicos: [`../versoes/V2_2.md`](../versoes/V2_2.md) · [`FLUXOS.md`](FLUXOS.md) §8.10.
...
|----------|:------------:|:--:|:---------:|:-------:|
| Onboarding / login | ✅ tour guiado v2.2 | ✅ tour guiado | ✅ tour guiado | ✅ tour guiado (+ MFA) |
