...
### Tenant inicial por modo

| Modo | Script | Tenant criado | Notas |
|------|--------|---------------|-------|
| **Demo** | `run-seed.ts` | **Clínica Horizonte** (+ VitaCare, TechCorp, massa PJ) | Nome alinhado à separação plataforma × clínica (v1.0.2) |
| **Operação** | `operation-bootstrap.ts` | **Clínica Bibi Saúde** (placeholder) | Bootstrap mínimo — renomeie em `/interno/branding` ou cadastros |

O nome **Clínica Bibi Saúde** no bootstrap de operação é apenas um rótulo inicial
editável; **não** representa a marca da plataforma na landing (`PLATFORM_BRANDING`
permanece **Sistema Bibi**). Usuários e catálogo de procedimentos são os mesmos
e-mails do demo (`dra.helena@bibi.health`, etc.) para facilitar o primeiro acesso.
