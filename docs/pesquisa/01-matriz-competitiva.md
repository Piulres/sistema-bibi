# Sistema Bibi - ServiceOS — Matriz Competitiva

**Benchmark competitivo estratégico 2026 — infraestrutura transacional multi-nicho**

| Meta | Valor |
|------|-------|
| Versão | 2.0 (ServiceOS) |
| Data | Junho/2026 |

---

## Reposicionamento estratégico

O **Sistema Bibi - ServiceOS** não compete como mais um ERP clínico. Posiciona-se como **infraestrutura transacional** que:

- Elimina a **"caixa preta" da sinistralidade** — cada procedimento/serviço é rastreado e cobrado por uso real.
- Oferece **transparência total** via quatro portais segregados (prestador, interno, PJ, beneficiário).
- Escala **horizontalmente** para saúde, veterinária, odontologia, jurídico, bem-estar e educação — mesmo motor Pay Per Use.

Concorrentes tradicionais vendem **elegibilidade** (vidas). O Bibi vende **infraestrutura de confiança** sobre transações efetivas.

---

## Concorrentes mais relevantes

1. Conexa
2. Vitta
3. Pipo Saúde
4. Alice
5. ERPMed / Centtralmed
6. Feegow
7. iClinic
8. Doctoralia
9. Sami
10. QSaúde

---

## Matriz comparativa

**Legenda:** ✅ Forte · 🟡 Parcial · ❌ Ausente · ⭐ Diferencial Bibi

| Critério | Bibi ServiceOS | Conexa | Vitta | Alice | Pipo | Feegow | iClinic | Doctoralia | Sami | QSaúde |
|----------|:----:|:------:|:-----:|:-----:|:----:|:------:|:-------:|:----------:|:----:|:------:|
| Pay Per Use nativo | ⭐✅ | 🟡 | 🟡 | ❌ | 🟡 | ❌ | ❌ | ❌ | ❌ | ❌ |
| Transparência de sinistralidade (anti caixa-preta) | ⭐✅ | 🟡 | 🟡 | ❌ | 🟡 | ❌ | ❌ | ❌ | ❌ | ❌ |
| Multi-nicho (ServiceOS) | ⭐✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| Precificação por Empresa | ⭐✅ | ❌ | 🟡 | ❌ | 🟡 | ❌ | ❌ | ❌ | ❌ | ❌ |
| Portal Empresa/RH | ⭐✅ | ✅ | ✅ | 🟡 | ✅ | ❌ | ❌ | ❌ | 🟡 | 🟡 |
| Portal Beneficiário | ⭐✅ | ✅ | ✅ | ✅ | 🟡 | ❌ | ❌ | ✅ | ✅ | ✅ |
| Portal Prestador | ⭐✅ | ✅ | 🟡 | ❌ | ❌ | ✅ | ✅ | ❌ | ❌ | ❌ |
| Portal Operacional Interno | ⭐✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ❌ | ✅ | ✅ |
| Multi Tenant | ⭐✅ | 🟡 | 🟡 | ❌ | 🟡 | ❌ | ❌ | ❌ | ❌ | ❌ |
| White Label + tema escuro | ⭐✅ | 🟡 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| Agenda / operação | ✅ | ✅ | ✅ | ✅ | ❌ | ✅ | ✅ | 🟡 | ✅ | ✅ |
| Prontuário / registro clínico | ✅ | 🟡 | 🟡 | ✅ | ❌ | ✅ | ✅ | ❌ | 🟡 | 🟡 |
| Telemedicina | ✅ | ✅ | ✅ | ✅ | ❌ | ✅ | ✅ | 🟡 | ✅ | ✅ |
| CRM B2B | ⭐✅ | 🟡 | 🟡 | ❌ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ |
| API Aberta | ⭐✅ | 🟡 | 🟡 | ❌ | ❌ | 🟡 | 🟡 | ❌ | ❌ | ❌ |
| Webhooks | ⭐✅ | 🟡 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| PIX Integrado | ⭐✅ | ❌ | ❌ | ❌ | ❌ | 🟡 | 🟡 | ❌ | ❌ | ❌ |
| TISS | ✅ | 🟡 | ❌ | ❌ | ❌ | ✅ | ✅ | ❌ | ❌ | ❌ |
| IA assistencial | 🟡 | 🟡 | 🟡 | 🟡 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| Gestão de Rede Credenciada | ⭐✅ | 🟡 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |

---

## Conclusão

O Sistema Bibi - ServiceOS ocupa um espaço pouco explorado:

- **Infraestrutura transacional** B2B (não operadora, não ERP vertical)
- Rede de prestadores com **cobrança por uso real**
- **Transparência de consumo** — elimina desperdício de ~91% vs modelo por vida (referência 500 colaboradores)
- Portal corporativo + **white label multi-nicho**
- Quatro portais como **estações de trabalho** configuráveis por tenant

Hoje não existe um líder claro que combine simultaneamente Pay Per Use nativo, multi-nicho e transparência total de sinistralidade.

---

Ver também: [`02-analise-mercado.md`](02-analise-mercado.md) · [`../V2_0_ARCHITECTURE.md`](../V2_0_ARCHITECTURE.md) · [`../BENCHMARK.md`](../BENCHMARK.md)
